...
about those two libraries and nothing else. Keeping it here means an integration adds support by
...
def _keep_causal_cache_length(vae) -> None:
    ...
    """
    if getattr(vae, "_distvae_cache_length_kept", False) or not hasattr(
        vae, "clear_cache"
    ):
        return
    ...
    recount()
    counts = {
        name: value for name, value in vars(vae).items() if name.endswith("conv_num")
    }
    if not counts:
        return
...
def parallelize_decoder(vae, vae_group) -> str:
    ...
    if name is None:
        raise ValueError(
            f"DistVAE cannot shard this VAE decoder ({type(vae).__name__}): no adapter "
            f"matches its decoder blocks. Use Diffusers VAE tiling to lower decode memory instead."
        )
