...
take has to be recognised before wrapping. These build each VAE class an integration loads and
...
CONFIGS = {
    name: config
    for name, (config, _, _) in test_vae_tiling.TestEverySupportedVAE.VAES.items()
}


def _vae_class(name):
    """Return an installed optional VAE class, or skip only the current test/subtest."""
    cls = getattr(diffusers, name, None)
    if cls is None:
        raise unittest.SkipTest(f"{name} is not in diffusers {diffusers.__version__}")
    return cls


# The adapter each class needs, or None where DistVAE has nothing for its decoder. Every class an
# integration loads is shardable as of DistVAE's
    ...
    def test_an_unavailable_optional_vae_is_skipped(self):
        with mock.patch.object(diffusers, "AutoencoderKLFlux2", None, create=True):
            with self.assertRaises(unittest.SkipTest):
                _vae_class("AutoencoderKLFlux2")

    def test_an_installed_vae_class_is_returned(self):
        self.assertIs(_vae_class("AutoencoderKL"), diffusers.AutoencoderKL)
...
class TestDecoderAdapterChoice(unittest.TestCase):
    ...
    def test_every_vae_class_gets_the_adapter_it_needs(self):
        for name, expected in EXPECTED.items():
            with self.subTest(vae=name):
                vae = _vae_class(name)(**CONFIGS[name])
    ...
    def test_an_ltx2_decoder_that_injects_noise_is_not_shardable(self):
        ...
        vae = _vae_class("AutoencoderKLLTX2Video")(
...
class TestEncoderAdapterChoice(unittest.TestCase):
    ...
    def test_the_two_halves_are_recognised_independently(self):
        ...
        # order the supported integrations shard them in.
...
class TestEncoderScaleFactor(unittest.TestCase):
    ...
    def test_flux2s_pair_of_patch_sizes_is_not_mistaken_for_a_ratio(self):
        ...
        vae = _vae_class("AutoencoderKLFlux2")(**CONFIGS["AutoencoderKLFlux2"])
    ...
    def test_a_two_d_encoder_is_counted_off_its_stages(self):
        ...
        self.assertEqual(
            vae_parallel.encoder_scale_factor(diffusers.AutoencoderKL(**config)), 4
        )
...
class TestUnshardableIsRefused(unittest.TestCase):
    ...
    def test_it_names_the_contract_and_points_at_the_alternative(self):
        # DistVAE now fits every VAE class an integration loads, so the refusal is provoked with
        ...
        self.assertIn("DistVAE cannot shard this VAE decoder", message)
        self.assertIn("Diffusers VAE tiling", message)
    ...
    def test_encoding_is_refused_for_a_vae_no_encoder_adapter_fits(self):
        ...
        # every VAE class an integration loads.
        vae = diffusers.AutoencoderKL(**CONFIGS["AutoencoderKL"])
...
class TestBothHalvesShardTogether(unittest.TestCase):
    ...
    def test_every_vae_class_shards_both_halves(self):
        for name, config in CONFIGS.items():
            with self.subTest(vae=name):
                vae = _vae_class(name)(**config).eval()
                expected = vae_parallel.encoder_adapter_name(vae)
                encoder, decoder = vae.encoder, vae.decoder
                # Integrations shard the decoder first, which is what makes the order matter.
                self.assertEqual(
                    vae_parallel.parallelize_decoder(vae, vae_group=None),
                    EXPECTED[name],
                )
...
class TestCausalCacheLength(unittest.TestCase):
    ...
    def _parallelize(self, vae, half="decoder"):
        with mock.patch.object(
            vae_parallel, "_adapter", return_value=_ConvLosingAdapter
        ):
            if half == "decoder":
                vae_parallel.parallelize_decoder(vae, vae_group=None)
            else:
    ...
    def test_a_recounting_vae_still_caches_one_entry_per_convolution(self):
        vae = _vae_class("AutoencoderKLQwenImage")(**CONFIGS["AutoencoderKLQwenImage"])
        vae.clear_cache()
