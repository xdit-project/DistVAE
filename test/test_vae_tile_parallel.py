...
def _by_runs(weights, world_size: int) -> List[int]:
    ...
    return [
        ...
        for rank, (start, stop) in enumerate(
            vae_tile_parallel.runs(weights, world_size)
        )
...
def _dispatch_in_a_group(
    rank: int, world_size: int, port: int, calls_made: int
) -> None:
    ...
    try:
        ...
        assert len(results) == len(
            indices
        ), f"{len(results)} results for {len(indices)} calls"
...
class TestParallelContext(unittest.TestCase):
    def test_metadata_is_distvae_owned_and_immutable(self):
        vae = SimpleNamespace()
        context = ParallelContext(group=None, rank=0, world_size=1, patch_dim=-2)

        vae_tile_parallel.mark(vae, context)

        self.assertIs(vae_tile_parallel.context_of(vae), context)
        self.assertIsNone(vae_tile_parallel.group_of(vae))
        self.assertEqual(vars(vae), {"_distvae_tile_parallel_context": context})

    def test_dispatch_uses_a_captured_context_without_global_lookup(self):
        context = ParallelContext(group=None, rank=0, world_size=1, patch_dim=-2)
        calls = [lambda: torch.tensor(1), lambda: torch.tensor(2)]

        results = vae_tile_parallel.dispatch_over(context)(calls)

        self.assertEqual([result.item() for result in results], [1, 2])


class TestDiffusersCompatibility(unittest.TestCase):
    def test_an_unavailable_run_vae_is_skipped_before_spawning(self):
        import diffusers

        with mock.patch.object(diffusers, "AutoencoderKLQwenImage", None, create=True):
            with self.assertRaises(unittest.SkipTest):
                _require_run_vae(self, "AutoencoderKLQwenImage")

    def test_an_installed_tiling_api_is_accepted(self):
        _require_run_vae(self, "AutoencoderKL")
...
def _require_run_vae(testcase, name: str) -> None:
    """Skip one parent subtest before spawning when its Diffusers VAE cannot tile."""
    import diffusers

    cls = getattr(diffusers, name, None)
    if cls is None:
        testcase.skipTest(f"{name} is not in diffusers {diffusers.__version__}")
    vae = cls(**RUN_VAES[name][0])
...
def _tiled_vae(name: str, device=None, overlap: Optional[float] = None):
    ...
    if cls is None:
        raise RuntimeError(
            f"{name} disappeared after the parent process availability check"
        )
    vae = cls(**kwargs).eval()
    vae.enable_tiling()
...
class TestRuns(unittest.TestCase):
    ...
    def test_levelling_never_leaves_a_rank_worse_off_than_the_runs_it_started_from(
        self,
    ):
        random.seed(13)
                ...
                for world_size in range(2, min(rows * columns, 5) + 1):
                    ...
                    for n in range(rows * columns):
                        ...
                        if row > 0 and owner[n - columns] != owner[n]:
                            reaches += [n - columns] + (
                                [n - columns - 1] if column else []
                            )
                        if column > 0 and owner[n - 1] != owner[n]:
                            reaches += [n - 1] + ([n - columns - 1] if row else [])
                        for at in reaches:
                            self.assertIn(
                                at, wanted, f"{rows}x{columns}/{world_size} at {n}"
                            )
    ...
    def test_tiles_that_do_not_overlap_at_all_still_assemble(self):
        # A caller can widen the stride until the tiles touch rather than overlap, and
